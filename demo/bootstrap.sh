...
curl -s -L -C - -b "oraclelicense=accept-securebackup-cookie" -O http://download.oracle.com/otn-pub/java/jdk/8u31-b13/jdk-8u31-linux-x64.tar.gz

echo -e "$ECHO_PREFIX Unzip JDK"
tar xzf jdk-8u31-linux-x64.tar.gz -C /opt
ln -s /opt/jdk1.8.0_31 /opt/jdk
...
echo -e "$ECHO_PREFIX Install Kibana 3"
curl -O https://download.elasticsearch.org/kibana/kibana/kibana-3.1.2.tar.gz
tar xzf kibana-3.1.2.tar.gz
mv kibana-3.1.2 /usr/share/nginx/www/kibana
rm kibana-3.1.2.tar.gz

echo -e "$ECHO_PREFIX Configure Kibana 3"
cp /vagrant/kibana3.config.js /usr/share/nginx/www/kibana/config.js

echo -e "$ECHO_PREFIX Install Kibana 3 dashboards"
...
echo -e "$ECHO_PREFIX Install Kibana 4 dashboards"
cp -r /vagrant/kibana4_es_config /var/lib/elasticsearch/
chown -R elasticsearch:elasticsearch /var/lib/elasticsearch/kibana4_es_config
curl -XPUT 'localhost:9200/_snapshot/es_backup' -d '{
    "type": "fs",
    "settings": {
        "location": "/var/lib/elasticsearch/kibana4_es_config",
        "compress": true
    }
}'
...
echo -e "$ECHO_PREFIX"
echo -e "$ECHO_PREFIX Kibana 3 is available at:"
...
echo -e "$ECHO_PREFIX"
echo -e "$ECHO_PREFIX Kibana 4 is available at:"
echo -e "$ECHO_PREFIX * http://localhost:15601"
